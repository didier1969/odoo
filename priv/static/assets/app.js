...
`);
  ...
  window.JSS = {
    ...
    startOptimizationTest: function() {
      console.log("\u{1F680} Starting JSS optimization test...");
    ...
    getSystemStatus: function() {
      fetch("/api/status").then((response) => response.json()).then((data) => {
        console.log("\u{1F4CA} System Status:", data);
      }).catch((error) => {
  ...
  function showNotification(type, message) {
    ...
    notification.className = `notification fixed top-4 right-4 px-4 py-2 rounded shadow-lg z-50 transition-all duration-300 ${type === "error" ? "bg-red-100 text-red-800 border border-red-200" : type === "success" ? "bg-green-100 text-green-800 border border-green-200" : type === "warning" ? "bg-yellow-100 text-yellow-800 border border-yellow-200" : "bg-blue-100 text-blue-800 border border-blue-200"}`;
